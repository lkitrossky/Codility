...
vector<int> solution(int K, int M, vector<int> &A) {
	int N = A.size();
	int leader = N / 2 + 1;
	//map<int, int> current; // Damian Werpachowski noted that this may result in O(M*log (M)) and not O(M). If map is replaced by vector it will be O(N+M), but for sparce array and small numbers it maybe even worse
	//New version with vector:  https://app.codility.com/cert/view/certRZ3FSF-4NBKB4YKYWUX2HK8/details/   also gold
	vector<int> current(M+2, 0); 
	vector<int> res;
	set<int> resSet;
//initialization
	for (int i = 0; i < K; i++) {
